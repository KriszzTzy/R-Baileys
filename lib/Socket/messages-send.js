...
const makeMessagesSocket = (config) => {
    ...
    const kriz = new imup(Utils_1, waUploadToServer, relayMessage, authState);
    ...
    const sendMessage = async (jid, content, options = {}) => {
        ...
        const messageType = kriz.detectType(content);
    ...
    switch(messageType) {
        case 'PAYMENT':
            const paymentContent = await kriz.handlePayment(content, quoted);
        ...
        case 'PRODUCT':
            const productContent = await kriz.handleProduct(content, jid, quoted);
        ...
        case 'INTERACTIVE':
            const interactiveContent = await kriz.handleInteractive(content, jid, quoted);
        ...
        case 'CAROUSEL_MESSAGE':
            const carouselMsgContent = await kriz.handleCarouselMessage(content, jid, quoted);
            const carouselMsgResult = await Utils_1.generateWAMessageFromContent(jid, carouselMsgContent, { quoted });
         return await relayMessage(jid, carouselMsgResult.message, {
        messageId: carouselMsgResult.key.id,
        ...getParticipantAttr()
          });
    
        case 'ALBUM':
            return await kriz.handleAlbum(content, jid, quoted);
        case 'EVENT':
            return await kriz.handleEvent(content, jid, quoted);
        case 'POLL_RESULT':
            return await kriz.handlePollResult(content, jid, quoted);
        case 'ORDER':
            return await kriz.handleOrderMessage(content, jid, quoted);
        case 'GROUP_STATUS':
            return await kriz.handleGroupStory(content, jid, quoted);
        case 'GROUP_LABEL':
            return await kriz.handleGbLabel(content, jid);
        case 'RICH_MESSAGE':
            return await kriz.handleRichMessage(content, jid, quoted);
    }
}
