...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        // handle botForwardedMessage type
        if (encNode && encNode.attrs.type === 'bot') {
            logger.debug({ key: node.attrs.key }, 'received bot/rich message');
        }
        let response;
    ...
    const requestPlaceholderResend = async (messageKey) => {
        ...
        await (0, Utils_1.delay)(3000); // dikurangi dari 5000 biar lebih cepat
        if (!placeholderResendCache.get(messageKey === null || messageKey === void 0 ? void 0 : messageKey.id)) {
    ...
    const handleCall = async (node) => {
        ...
        // emit interactiveResponseMessage biar bot bisa handle button press
        if (msg?.message?.interactiveResponseMessage) {
            const nativeFlow = msg.message.interactiveResponseMessage.nativeFlowResponseMessage;
            if (nativeFlow?.name) {
                logger.debug({ name: nativeFlow.name }, 'received interactive response');
            }
        }
        if (status === 'offer') {
